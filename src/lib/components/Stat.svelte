  ...
  <g transform='translate(0,{indicatorHeight/2})'>
    ...
    {#if regio !== 'Nederland'}
      <g class='hoveryear_{indicator.title.replaceAll(' ', '')}' transform='translate({xScaleStats(medianValueOtherYear)+175},0)' style='visibility:hidden'>
        <line y1='-0.5em' y2={indicatorHeight*0.06} stroke='steelblue' stroke-width='5' stroke-linecap="round"/>
        <text style='fill:steelblue; font-size:10px' text-anchor={'middle'} dx={0} dy='1.25em'>{Math.round(medianValueOtherYear*10)/10}</text>
      </g>
    {/if}
